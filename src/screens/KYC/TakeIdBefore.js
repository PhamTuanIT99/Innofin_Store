...
export default function TakeBefore() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#606060', alignItems: 'center' }}>
      ...
      <TouchableOpacity
        ...
        onPress={async () => {
          if (cameraRef) {
            let photoBefore = await cameraRef.takePictureAsync({ skipProcessing: true });
            let reSize = async () => {
              const setImg = await ImageManipulator.manipulateAsync(photoBefore.uri, [], {
                compress: Platform.OS === 'ios' ? 0 : 0.5,
              });
              let B = {
                uri: setImg.uri,
                type: getImageType(
                  photoBefore.uri.substring(
                    photoBefore.uri.lastIndexOf('.') + 1,
                    photoBefore.uri.length,
                  ),
                ),
                name: photoBefore.uri.substring(
                  photoBefore.uri.lastIndexOf('/') + 1,
                  photoBefore.uri.length,
                ),
              };
              navigaton.navigate('CmndBefore', { B });
            };
            reSize();
          }
        }}>
